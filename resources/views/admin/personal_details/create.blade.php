...
<section class="contact" style="padding-top:40px;min-height:70vh">
  ...
  <div class="container" style="max-width:700px;margin:0 auto">
    <form method="POST" action="{{ route('personal_details.store') }}" class="form" style="display:flex;flex-direction:column;gap:16px">
      @csrf
      <div class="input-box"><input name="department" placeholder="Department" value="{{ old('department') }}"></div>
      ...
      <div class="input-box" style="display:flex;gap:12px">
        <input type="number" name="age" min="0" max="200" placeholder="Age" value="{{ old('age') }}">
        <input type="date" name="dob" value="{{ old('dob') }}">
        <input name="blood_group" placeholder="Blood Group" value="{{ old('blood_group') }}">
      </div>
      <div class="input-box"><input name="gender" placeholder="Gender" value="{{ old('gender') }}"></div>
