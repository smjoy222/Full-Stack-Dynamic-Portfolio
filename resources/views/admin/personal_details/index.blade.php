...
<section class="services" id="personal-details" style="padding-top:40px;min-height:70vh">
  <div class="main-text">
    <span>Manage</span>
    <h2>Personal Details</h2>
  </div>
  <div style="margin: 0 auto; max-width: 1000px; display:flex; justify-content: flex-end;">
    <a href="{{ route('personal_details.create') }}" class="btn">Add</a>
  </div>
  <div class="section-services" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:20px;max-width:1000px;margin:20px auto">
    ...
    <div class="service-box">
      ...
      <p>{{ $item->address }} | Age: {{ $item->age }}</p>
      <div style="display:flex;gap:10px">
        <a href="{{ route('personal_details.edit', $item) }}" class="read">Edit</a>
        <form action="{{ route('personal_details.destroy', $item) }}" method="POST" onsubmit="return confirm('Delete?')">
          @csrf
          @method('DELETE')
