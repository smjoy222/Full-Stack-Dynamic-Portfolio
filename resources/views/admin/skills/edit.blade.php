...
<section class="contact" style="padding-top:80px;min-height:70vh">
  <div class="main-text">
    <span>EDIT</span>
    <h2 style="color:#12f7ff">Update Skill</h2>
  </div>
  <div class="container" style="max-width:700px;margin:0 auto;background:rgba(41, 46, 51, 0.7);border-radius:10px;padding:30px;box-shadow:0 0 20px rgba(18, 247, 255, 0.2);border:1px solid rgba(18, 247, 255, 0.1)">
    <form method="POST" action="{{ route('skills.update', $skill) }}" class="form" enctype="multipart/form-data" style="display:flex;flex-direction:column;gap:20px">
      @csrf
      @method('PUT')
      
      <div class="form-group">
        <label style="color:#12f7ff;font-size:14px;margin-bottom:5px;display:block">Skill Name</label>
        <input name="name" placeholder="Enter skill name" value="{{ old('name', $skill->name) }}" required style="width:100%;padding:12px;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(18, 247, 255, 0.3);border-radius:5px;font-size:15px;transition:all 0.3s">
      </div>
