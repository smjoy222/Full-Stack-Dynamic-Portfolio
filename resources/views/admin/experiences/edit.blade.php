...
<section class="contact" style="padding-top:80px;min-height:70vh">
  <div class="main-text">
    <span>EDIT</span>
    <h2 style="color:#12f7ff">Update Experience</h2>
  </div>
  <div class="container" style="max-width:700px;margin:0 auto;background:rgba(41, 46, 51, 0.7);border-radius:10px;padding:30px;box-shadow:0 0 20px rgba(18, 247, 255, 0.2);border:1px solid rgba(18, 247, 255, 0.1)">
    <form method="POST" action="{{ route('experiences.update', $experience) }}" class="form" style="display:flex;flex-direction:column;gap:20px">
      @csrf
      @method('PUT')
      
      <div class="form-group">
        <label style="color:#12f7ff;font-size:14px;margin-bottom:5px;display:block">Designation</label>
        <input name="designation" placeholder="Enter job title or position" value="{{ old('designation', $experience->designation) }}" required style="width:100%;padding:12px;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(18, 247, 255, 0.3);border-radius:5px;font-size:15px;transition:all 0.3s">
      </div>
      
      <div class="form-group">
        <label style="color:#12f7ff;font-size:14px;margin-bottom:5px;display:block">Organization</label>
        <input name="organization" placeholder="Enter company or organization name" value="{{ old('organization', $experience->organization) }}" required style="width:100%;padding:12px;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(18, 247, 255, 0.3);border-radius:5px;font-size:15px;transition:all 0.3s">
      </div>
      ...
      <div style="display:flex;gap:20px;flex-wrap:wrap">
        <div class="form-group" style="flex:1;min-width:200px">
          ...
          <input type="date" name="from_date" value="{{ old('from_date', optional($experience->from_date)->format('Y-m-d')) }}" required style="width:100%;padding:12px;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(18, 247, 255, 0.3);border-radius:5px;font-size:15px;transition:all 0.3s">
        </div>
        
        <div class="form-group" style="flex:1;min-width:200px">
          <label style="color:#12f7ff;font-size:14px;margin-bottom:5px;display:block">To Date (leave empty for current)</label>
          <input type="date" name="to_date" value="{{ old('to_date', optional($experience->to_date)->format('Y-m-d')) }}" style="width:100%;padding:12px;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(18, 247, 255, 0.3);border-radius:5px;font-size:15px;transition:all 0.3s">
        </div>
      </div>
      
      <div class="form-group">
        <label style="color:#12f7ff;font-size:14px;margin-bottom:5px;display:block">Description</label>
        <textarea name="description" placeholder="Enter experience description (responsibilities, achievements, etc.)" rows="5" style="width:100%;padding:12px;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(18, 247, 255, 0.3);border-radius:5px;font-size:15px;transition:all 0.3s;resize:vertical;">{{ old('description', $experience->description) }}</textarea>
      </div>
